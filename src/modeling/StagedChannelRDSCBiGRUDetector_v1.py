...
class ChannelStage(nn.Module):
    """
    Several RDSC blocks applied independently to each physical EEG channel.
    ...
    def __init__(self, num_phys_channels: int, hidden_per_channel: int,
                 kernel_sizes=(9, 9, 7), dropout: float = 0.15):
        super().__init__()
        self.branches = nn.ModuleList([
            nn.Sequential(*[
                ChannelRDSCBlock(hidden_per_channel, kernel_size=k, dropout=dropout)
...
class WeakChannelMix(nn.Module):
    ...
    Mixing is feature-wise: for each hidden feature h, only the three physical
    channels are mixed with each other. Hidden features are not globally mixed.
    Initial transform is exactly identity because mix weights are zero.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, P, H, T)
        weight = self.mix * self.offdiag_mask  # no self-channel term here
        mixed = torch.einsum("hoq,bqht->boht", weight, x)
        return x + self.alpha * mixed


@register_model("StagedChannelRDSCBiGRUDetector_v1")
class StagedChannelRDSCBiGRUDetector_v1(nn.Module):
    """
    Channel-specific staged RDSC + weak channel mixing + BiGRU.

    Main difference from RDSCBiGRUDetector:
      - no raw input normalization;
      - physical EEG channels are kept isolated through the stem and stage blocks;
      - two weak residual cross-channel mixes are inserted between isolated stages;
    ...
    def __init__(self, input_channels: int = 3, hidden_per_channel: int = 8,
                 final_hidden: int = 32, gru_hidden: int = 32,
                 gru_layers: int = 1, dropout: float = 0.15):
        super().__init__()
        if input_channels != 3:
            raise ValueError(
                "StagedChannelRDSCBiGRUDetector_v1 is currently designed for "
        ...
        # Per-physical-channel stems: each channel starts as its own branch.
        ...
        # Isolated channel processing, then weak cross-channel exchange.
        self.stage1 = ChannelStage(input_channels, hidden_per_channel,
                                   kernel_sizes=(9, 9, 7), dropout=dropout)
        ...
        # Strong final fusion: after channel-specific processing.
