#!/usr/bin/env python3
"""
Скрипт для извлечения данных из логов TensorBoard для анализа экспериментов.
"""
import os
import pandas as pd
...
def extract_scalars_from_event_file(event_file_path):
    """
    Извлекает скалярные метрики из файла событий TensorBoard.
    ...
    # Получаем все доступные скалярные метрики
    scalar_tags = event_acc.Tags()['scalars']
    
    # Извлекаем данные для каждой метрики
    metrics_data = {}
    for tag in scalar_tags:
        events = event_acc.Scalars(tag)
        # Берем только последние значения (последняя эпоха)
        if events:
            # Получаем все значения для метрики
            values = [event.value for event in events]
            # Сохраняем последние значение и лучшее значение
            metrics_data[tag] = {
                'last': values[-1] if values else None,
                'best': max(values) if 'acc' in tag or 'f1' in tag or 'precision' in tag or 'recall' in tag else min(values) if 'loss' in tag else values[-1] if values else None,
                'values': values
            }
...
def extract_experiment_data(log_dir, experiment_name):
    ...
    # Обычно у нас есть несколько файлов событий, берем последний (самый полный)
    if event_files:
        # Сортируем по времени создания и берем последний
        event_files.sort(key=os.path.getctime)
        latest_event_file = event_files[-1]
        experiment_data['metrics'] = extract_scalars_from_event_file(latest_event_file)
...
def generate_comparison_table(experiments_data):
    ...
    table = "| Модель | val_loss | val_acc | val_f1 | test_acc | test_f1 |\n"
    table += "|--------|----------|---------|--------|----------|---------|\n"
    ...
    for exp_data in experiments_data:
        ...
        row += f"| {val_loss_str} | {val_acc_str} | {val_f1_str} | {test_acc_str} | {test_f1_str} |"
        table += row + "\n"
...
def main():
    ...
    # Сводный отчет
    with open("experiments_comparison_summary.md", 'w', encoding='utf-8') as f:
        f.write("# Сравнительный анализ экспериментов\n\n")
    ...
    print("Создан сводный отчет: experiments_comparison_summary.md")
